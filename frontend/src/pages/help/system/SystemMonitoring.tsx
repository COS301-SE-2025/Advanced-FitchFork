...
export default function SystemMonitoringHelp() {
  ...
  return (
    <Space direction="vertical" size="small" className="w-full !p-0">
      <Title level={2} className="mb-0">System Monitoring &amp; Health</Title>

      <section id="overview" className="scroll-mt-24" />
      <Title level={3}>Overview</Title>
      <Paragraph className="mb-0">
        Live system health is broadcast over WebSockets and rendered in the header and System
        admin page. Use it to spot load spikes, memory pressure, or long submission queues.
      </Paragraph>
      ...
      <Paragraph>
        ...
        <Text code> 1</Text>, <Text code>5</Text>, and <Text code>15</Text> minutes. Values near your CPU core count
        suggest the machine is fully utilized; sustained values well above core count may indicate saturation.
      </Paragraph>

      <section id="cpu" className="scroll-mt-24" />
      <Title level={3}>CPU</Title>
      <Paragraph>
        <Text strong>Cores</Text> is the logical CPU count. <Text strong>Avg usage</Text> is the mean of per-core
        utilization sampled periodically.
      </Paragraph>

      <section id="memory" className="scroll-mt-24" />
      <Title level={3}>Memory</Title>
      <Paragraph>
        Includes total/used memory and swap usage where applicable. Values appear in human-friendly units.
      </Paragraph>

      <section id="disks" className="scroll-mt-24" />
      <Title level={3}>Disks</Title>
      <Paragraph>
        Each mounted disk shows its filesystem type, available space, and total capacity. Low available space may
        cause failures or slowdowns; consider cleaning temporary files or expanding storage.
      </Paragraph>

      <section id="code-manager" className="scroll-mt-24" />
      <Title level={3}>Code Manager (Submissions)</Title>
      <Paragraph>
        The submission runtime is capped by <Text strong>Max Concurrent</Text>. <Text strong>Running</Text> shows active
        containers; <Text strong>Waiting</Text> is queued submissions. Adjust concurrency to balance throughput vs.
        system load.
      </Paragraph>
    </Space>
  );
