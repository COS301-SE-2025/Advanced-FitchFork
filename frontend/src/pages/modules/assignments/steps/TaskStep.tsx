...
const TasksLayout = () => {
  ...
  const { refreshReadiness } = useAssignment();
  ...
  useEffect(() => {
    if (!module.id || !assignment.id || !readiness) return;
    ...
    listTasks(module.id, assignment.id)
      .then((res) => {
        if (res.success) {
          ...
          if (endsWithTasks && sorted.length > 0 && readiness.tasks_present) {
            console.log(
              '[REDIRECT] location ends with /tasks, redirecting to first task ID:',
              sorted[0].id,
            );
            navigate(
              `/modules/${module.id}/assignments/${assignment.id}/steps/tasks/${sorted[0].id}`,
              {
                replace: true,
              },
            );
  ...
  }, [module.id, assignment.id, readiness?.tasks_present, location.pathname]);
  ...
  const onClickNextStep = async () => {
    await refreshReadiness?.();
    if (readiness?.tasks_present) {
      goToStep('memo-output');
    } else {
      notifyError('Cannot continue', 'You must have at least one task created.');
    }
  };
      ...
      <div className="flex">
        ...
        <div className="w-[240px] bg-white dark:bg-gray-950 border-r border-gray-200 dark:border-gray-800 px-2 py-4">
          <Menu
            ...
            onClick={({ key }) => {
              navigate(`/modules/${module.id}/assignments/${assignment.id}/steps/tasks/${key}`);
            }}
            items={menuItems}
