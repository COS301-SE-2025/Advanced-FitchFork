...
use chrono::Utc;
use db::{
    models::plagiarism_case::{ActiveModel, Column, Entity},
...
impl ToActiveModel<Entity> for CreatePlagiarismCase {
    async fn into_active_model(self) -> Result<ActiveModel, AppError> {
        ...
        Ok(ActiveModel {
            ...
            lines_matched: Set(self.lines_matched),
            created_at: Set(now),
            updated_at: Set(now),
            ..Default::default()
        })
...
impl<'a> Service<'a, Entity, Column, CreatePlagiarismCase, UpdatePlagiarismCase>
    for PlagiarismCaseService
{
