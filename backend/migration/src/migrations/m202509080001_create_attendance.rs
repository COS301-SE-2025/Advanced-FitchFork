...
impl MigrationName for Migration {
    fn name(&self) -> &str { "m202509080001_create_attendance" }
}
...
impl MigrationTrait for Migration {
        ...
        manager
            .create_table(
                Table::create()
                    ...
                    .col(ColumnDef::new(Alias::new("module_id")).big_integer().not_null())
                    .col(ColumnDef::new(Alias::new("created_by")).big_integer().not_null())
                    .col(ColumnDef::new(Alias::new("title")).string().not_null())
                    .col(ColumnDef::new(Alias::new("active")).boolean().not_null().default(false))
                    .col(ColumnDef::new(Alias::new("rotation_seconds")).integer().not_null().default(30))
                    // REMOVED: code_length
                    .col(ColumnDef::new(Alias::new("secret")).string_len(64).not_null())
                    .col(ColumnDef::new(Alias::new("restrict_by_ip")).boolean().not_null().default(false))
                    .col(ColumnDef::new(Alias::new("allowed_ip_cidr")).string().null())
                    .col(ColumnDef::new(Alias::new("created_from_ip")).string().null())
        ...
        manager
            .create_table(
                Table::create()
                    ...
                    .col(ColumnDef::new(Alias::new("taken_at")).timestamp().not_null().default(Expr::cust("CURRENT_TIMESTAMP")))
                    .col(ColumnDef::new(Alias::new("ip_address")).string().null())
                    // REMOVED: method
                    .col(ColumnDef::new(Alias::new("token_window")).big_integer().not_null())
                    .primary_key(Index::create().col(Alias::new("session_id")).col(Alias::new("user_id")))
                    .foreign_key(
                        ForeignKey::create()
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager.drop_table(Table::drop().table(Alias::new("attendance_records")).to_owned()).await?;
        manager.drop_table(Table::drop().table(Alias::new("attendance_sessions")).to_owned()).await
    }
}
