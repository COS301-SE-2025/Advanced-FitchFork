...
mod tests {
    ...
    let allocator_json = serde_json::json!({
        ...
        fs::write(&alloc_path, serde_json::to_string_pretty(&allocator_json).unwrap()).unwrap();
        ...
        assert_eq!(report.mark.total, 10.0);
        assert_eq!(report.mark.earned, 10.0, "reorder_by_memo should align lines and award full marks");
    }
    ...
    async fn test_regex_scheme_ignores_reorder_flag() {
        ...
        let job1 = MarkingJob::new(memo_outputs.clone(), student_outputs.clone(), allocator.clone(), cfg1);
        ...
        assert_eq!(r1.mark.earned, r2.mark.earned, "Regex scheme should ignore reordering");
    }
