...
impl Model {
    ...
    ///
    /// # Arguments
    /// * `conn` - Reference to the database connection.
...
mod tests {
    ...
    async fn test_create_module() {
        ...
        let module = Model::create(code, year, description, credits)
            .await
    ...
    async fn test_edit_module() {
        let initial = Model::create("COS132", 2024, Some("Old Desc"), 12)
            .await
            .unwrap();

        let updated = Model::edit(initial.id, "COS133", 2025, Some("New Desc"), 14)
            .await
