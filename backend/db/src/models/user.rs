...
mod tests {
    ...
    async fn test_create_and_get_user() {
        ...
        let _user = Model::create(username, email, password, false)
            .await
            .expect("Failed to create user");

        let found = Model::get_by_username(username)
            .await
    ...
    async fn test_verify_credentials_success_and_failure() {
        ...
        Model::create(username, email, password, false)
            .await
            .expect("Failed to create user");

        let ok = Model::verify_credentials(username, password)
            .await
            .expect("Failed to verify credentials");
        assert!(ok.is_some());

        let bad = Model::verify_credentials(username, "wrong_pw")
            .await
    ...
    async fn test_is_in_role_and_get_module_roles() {
        ...
        // Create user
        let user = Model::create("u00001111", "modrole@example.com", "pw", false)
            .await
            .expect("create user");
        ...
        // Check `is_in_role`
        let is_lecturer = Model::is_in_role(user.id, module.id, "lecturer")
            .await
            .expect("check is_in_role");
        assert!(is_lecturer);

        let is_tutor = Model::is_in_role(user.id, module.id, "tutor")
            .await
            .expect("check is_in_role");
        assert!(!is_tutor);

        // Check `get_module_roles`
        let roles = Model::get_module_roles(user.id)
            .await
