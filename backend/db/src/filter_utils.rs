use sea_orm::{ColumnTrait, Condition, DbErr, QueryOrder, prelude::Expr};
        ...
        C: ColumnTrait,
    {
        ...
        match (&filter_param.value, &filter_param.operator) {
            ...
            (FilterValue::String(values), CompareOp::Like) => {
                ...
                Ok(
                    condition
                        .add(Expr::cust(&format!("LOWER({})", column.as_str())).like(&pattern)),
                )
            }
            (FilterValue::String(values), CompareOp::Gt) => {
                if values.len() != 1 {
                    return Err(DbErr::Custom(
                        "Greater than comparison requires exactly one value".to_string(),
                    ));
                }
            ...
            (FilterValue::String(values), CompareOp::Gte) => {
                if values.len() != 1 {
                    return Err(DbErr::Custom(
                        "Greater than or equal comparison requires exactly one value".to_string(),
                    ));
                }
            ...
            (FilterValue::String(values), CompareOp::Lt) => {
                if values.len() != 1 {
                    return Err(DbErr::Custom(
                        "Less than comparison requires exactly one value".to_string(),
                    ));
                }
            ...
            (FilterValue::String(values), CompareOp::Lte) => {
                if values.len() != 1 {
                    return Err(DbErr::Custom(
                        "Less than or equal comparison requires exactly one value".to_string(),
                    ));
                }
...
impl QueryUtils {
        ...
        C: ColumnTrait,
    {
        ...
        if query_param.columns.is_empty() {
            return Err(DbErr::Custom(
                "Query parameter must specify at least one column".to_string(),
            ));
        }
        ...
        for column_name in &query_param.columns {
            ...
            or_condition =
                or_condition.add(Expr::cust(&format!("LOWER({})", column.as_str())).like(&pattern));
        }
