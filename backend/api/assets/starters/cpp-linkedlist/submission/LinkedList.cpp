...
{
    copyFrom(other); // BUG: copyFrom uses push_front => reverses order
}
...
LinkedList &LinkedList::operator=(const LinkedList &other)
{
    ...
    {
        clear();
        copyFrom(other); // BUG: reversed order
    }
    return *this;
}
...
bool LinkedList::pop_front(int &out)
{
    ...
    out = n->value;
    head_ = n->next;
    // BUG: when list becomes empty, tail_ should be set to nullptr (not done)
    delete n;
    --size_;
    return true;
}
...
bool LinkedList::erase(std::size_t index)
{
    ...
    {
        int tmp;
        return pop_front(tmp); // inherits tail_ bug
    }
    auto *prev = head_;
    for (std::size_t i = 0; i + 1 < index; ++i)
        prev = prev->next;
    ...
    // BUG: if we removed the last element, tail_ should move to prev (not done)
    delete victim;
    --size_;
    return true;
}
...
// BUGGY: copies in reverse by using push_front
void LinkedList::copyFrom(const LinkedList &other)
{
    for (auto *n = other.head_; n; n = n->next)
    {
        push_front(n->value); // reverses order vs. 'other'
    }
}
