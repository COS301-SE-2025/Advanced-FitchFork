...
static void task1_basic_ops(void) {
    ...
    section("front_back");
    int f=0,b=0; ll_front(&lst,&f); ll_back(&lst,&b);
    printf("front=%d back=%d\n", f, b);

    section("pop_front");
    int popped=0; int ok = ll_pop_front(&lst,&popped);
    ...
    LinkedList one; ll_init(&one);
    ll_push_back(&one, 7);
    int popped2=0; int ok2 = ll_pop_front(&one,&popped2);
...
static void task2_insert_erase(void) {
    section("start-task2");
    LinkedList lst; ll_init(&lst);
    for (int i=1;i<=5;i++) ll_push_back(&lst, i);
    print_list(&lst, "seed");
...
static void task3_copy_move(void) {
    section("start-task3");
    LinkedList a; ll_init(&a);
    for (int i=0;i<4;i++) ll_push_back(&a, i*10);
    print_list(&a, "a");
    ...
    LinkedList d; ll_init(&d);
    ll_move_assign_from(&d, &c);
    print_list(&d, "d");
    print_list(&c, "c-moved-from");

    ll_clear(&a); ll_clear(&b); ll_clear(&c); ll_clear(&d);
}

int main(int argc, char** argv) {
    const char* which = argc >= 2 ? argv[1] : "";
    if (strcmp(which, "task1") == 0) { task1_basic_ops(); return 0; }
    if (strcmp(which, "task2") == 0) { task2_insert_erase(); return 0; }
    if (strcmp(which, "task3") == 0) { task3_copy_move(); return 0; }
