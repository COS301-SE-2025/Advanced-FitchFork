...
func (l *LinkedList) Clear() { l.head, l.tail = nil, nil }
...
// BUG: returns head instead of tail
...
func (l *LinkedList) RemoveAt(idx int) bool {
    if idx < 0 || idx >= l.size { return false }
    if idx == 0 { ok, _ := l.PopFront(); return ok }
    prev := l.head
    for i := 0; i < idx-1; i++ { prev = prev.next }
    victim := prev.next
