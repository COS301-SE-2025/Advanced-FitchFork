...
pub fn assignment_routes() -> Router {
    Router::new()
        .route("/", post(create_assignment).route_layer(from_fn(require_lecturer)))
        ...
        .route("/{assignment_id}", put(edit_assignment).route_layer(from_fn(require_lecturer)))
        .route("/{assignment_id}", delete(delete_assignment).route_layer(from_fn(require_lecturer)))
        .route("/{assignment_id}/open", put(open_assignment).route_layer(from_fn(require_lecturer)))
        .route("/{assignment_id}/close", put(close_assignment).layer(from_fn(require_lecturer)))
        .route("/bulk", delete(bulk_delete_assignments).layer(from_fn(require_lecturer)))
        .route("/bulk", put(bulk_update_assignments).layer(from_fn(require_lecturer)))
        .route("/{assignment_id}/stats", get(get_assignment_stats).route_layer(from_fn(require_lecturer)))
        .route("/{assignment_id}/readiness", get(get_assignment_readiness).route_layer(from_fn(require_lecturer)))
        .nest("/{assignment_id}/tasks", tasks_routes().route_layer(from_fn(require_lecturer)))
        .nest("/{assignment_id}/config", config_routes().layer(from_fn(require_lecturer)))
        .nest("/{assignment_id}/memo_output", memo_output_routes().layer(from_fn(require_lecturer)))
        .nest("/{assignment_id}/mark_allocator", mark_allocator_routes().route_layer(from_fn(require_lecturer)))
