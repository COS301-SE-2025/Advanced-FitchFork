...
//!
//! Routes include:
//! - Create, resubmit, remark, and get submissions
    ...
    Router::new()
        ...
        .route(
            ...
            get(get_submission_output)
                .route_layer(from_fn_with_state(app_state.clone(), require_lecturer_or_tutor)),
        )
        .route(
            "/",
            post(submit_assignment)
                .route_layer(from_fn_with_state(app_state.clone(), require_ready_assignment)),
        )
        .route(
            "/remark",
            post(remark_submissions)
