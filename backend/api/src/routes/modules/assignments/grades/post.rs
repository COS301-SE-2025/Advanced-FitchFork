...
use sea_orm::{
    ...
    Set, TransactionTrait, Order, QuerySelect, DbErr,
};
use serde::Serialize;
...
use db::models::{
    assignment::{Entity as AssignmentEntity, Column as AssignmentCol, Model as Assignment},
    assignment_submission::{Entity as SubmissionEntity, Column as SubmissionCol, Model as Submission},
    grade::{Entity as GradeEntity, ActiveModel as GradeActive, Column as GradeCol},
    user_module_role::{Entity as UMR, Column as UMRCol},
};

use db::models::assignment_file::{Entity as AssignmentFileEntity, Column as AFCol, FileType};
...
/// Returns "best" or "last" (defaults to "best") from assignment config on disk.
fn grading_policy_str(module_id: i64, assignment_id: i64) -> String {
    ExecutionConfig::get_execution_config(module_id, assignment_id)
        .ok()
        .and_then(|cfg| cfg.grading.and_then(|g| g.policy))
        .map(|s| s.to_ascii_lowercase())
...
/// Pick a student's submission according to policy:
/// - Best: highest score (ties broken by newest created_at)
/// - Last: newest submission by created_at
/// Returns (submission_id, score) or None if no submissions exist.
async fn pick_submission_for_student(
    ...
    let mut q = SubmissionEntity::find()
        ...
        .filter(SubmissionCol::user_id.eq(student_id));
