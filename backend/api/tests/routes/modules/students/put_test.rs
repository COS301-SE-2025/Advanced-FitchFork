...
mod tests {
    ...
    async fn setup_test_data() -> TestData {
        let module = ModuleModel::create("COS401", 2024, Some("Test Module"), 16).await.unwrap();
        ...
        let student1 = UserModel::create("student1", "student1@test.com", "password1", false).await.unwrap();
        let lecturer1 = UserModel::create("lecturer1", "lecturer1@test.com", "password2", false).await.unwrap();
        let tutor1 = UserModel::create("tutor1", "tutor1@test.com", "password3", false).await.unwrap();
    ...
    /// Test Case 1: Successful update of roles to student by admin
    #[tokio::test]
    async fn test_put_students_success_as_admin() {
        ...
        let req = Request::builder()
            ...
            .body(Body::from(json!({ "user_ids": [data.student1.id, data.lecturer1.id, data.tutor1.id] }).to_string()))
            .unwrap();
        ...
        assert_eq!(json["message"], "Users set as students successfully");
        let roles = UserModuleRoleModel::get_users_by_module_role(data.module.id as i32, Role::Student).await.unwrap();
    ...
    #[tokio::test]
    async fn test_put_students_forbidden_for_non_admin() {
        dotenvy::dotenv().expect("Failed to load .env");
        let data = setup_test_data().await;
    ...
    #[tokio::test]
    async fn test_put_students_module_not_found() {
        dotenvy::dotenv().expect("Failed to load .env");
        let data = setup_test_data().await;
    ...
    #[tokio::test]
    async fn test_put_students_user_not_found() {
        dotenvy::dotenv().expect("Failed to load .env");
        let data = setup_test_data().await;
    ...
    #[tokio::test]
    async fn test_put_students_user_not_assigned() {
        dotenvy::dotenv().expect("Failed to load .env");
        let data = setup_test_data().await;
    ...
    #[tokio::test]
    async fn test_put_students_empty_user_list() {
        dotenvy::dotenv().expect("Failed to load .env");
        let data = setup_test_data().await;
