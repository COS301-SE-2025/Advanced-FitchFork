...
mod tests {
    ...
    async fn setup(db: &sea_orm::DatabaseConnection) -> TestCtx {
        ...
        let _other_module = ModuleModel::create(
            db,
            "ATT302",
            Utc::now().year(),
            Some("Other Module"),
        ...
        // Users
        let lecturer      = UserModel::create(db, "del_lect",      "del_lect@test.com",      "password", false).await.unwrap();
        let other_lecturer= UserModel::create(db, "del_lect2",     "del_lect2@test.com",     "password", false).await.unwrap();
        let student       = UserModel::create(db, "del_student",   "del_student@test.com",   "password", false).await.unwrap();
        let tutor         = UserModel::create(db, "del_tutor",     "del_tutor@test.com",     "password", false).await.unwrap();
        let forbidden     = UserModel::create(db, "del_forbidden", "del_forbidden@test.com", "password", false).await.unwrap();
        ...
        UserModuleRoleModel::assign_user_to_module(db, other_lecturer.id, _other_module.id, Role::Lecturer).await.unwrap();
        ...
        TestCtx { lecturer, student, tutor, forbidden, module, _other_module, other_lecturer, session }
    }
    ...
    async fn test_delete_session_not_found_returns_404() {
        ...
        let (token, _) = generate_jwt(ctx.lecturer.id, ctx.lecturer.admin);
        let uri = format!("/api/modules/{}/attendance/sessions/{}", ctx.module.id, unknown);
        ...
        let msg = json["message"].as_str().unwrap_or_default().to_ascii_lowercase();
