...
mod tests {
    ...
    async fn setup_test_data() -> TestData {
        let module = ModuleModel::create("COS604", 2024, Some("Test Module"), 16).await.unwrap();
        ...
        let tutor2 = UserModel::create("tutor2", "tutor2@test.com", "password2", false).await.unwrap();
        let outsider = UserModel::create("outsider", "outsider@test.com", "password3", false).await.unwrap();
        UserModuleRoleModel::assign_user_to_module(tutor1.id, module.id, Role::Tutor).await.unwrap();
        UserModuleRoleModel::assign_user_to_module(tutor2.id, module.id, Role::Tutor).await.unwrap();
    ...
    /// Test Case 1: Successful removal of tutors by admin
    #[tokio::test]
    async fn test_delete_tutors_success_as_admin() {
        ...
        let req = Request::builder()
            ...
            .body(Body::from(json!({ "user_ids": [data.tutor1.id, data.tutor2.id] }).to_string()))
            .unwrap();
        ...
        assert_eq!(json["message"], "Tutors removed from module successfully");
        ...
        assert!(!ids.contains(&data.tutor2.id));
    }

    /// Test Case 2: Forbidden for non-admin users
    #[tokio::test]
    async fn test_delete_tutors_forbidden_for_non_admin() {
        dotenvy::dotenv().expect("Failed to load .env");
        let data = setup_test_data().await;
    ...
    #[tokio::test]
    async fn test_delete_tutors_module_not_found() {
        dotenvy::dotenv().expect("Failed to load .env");
        let data = setup_test_data().await;
    ...
    #[tokio::test]
    async fn test_delete_tutors_user_not_found() {
        dotenvy::dotenv().expect("Failed to load .env");
        let data = setup_test_data().await;
    ...
    /// Test Case 5: User not a tutor in the module (conflict)
    #[tokio::test]
    async fn test_delete_tutors_user_not_a_tutor() {
        dotenvy::dotenv().expect("Failed to load .env");
        let data = setup_test_data().await;
        ...
        let json: Value = serde_json::from_slice(&body).unwrap();
        assert_eq!(json["success"], false);
        assert_eq!(json["message"], "Some users are not tutors for this module");
    }

    /// Test Case 6: Empty user_ids list (bad request)
    #[tokio::test]
    async fn test_delete_tutors_empty_user_list() {
        dotenvy::dotenv().expect("Failed to load .env");
        let data = setup_test_data().await;
