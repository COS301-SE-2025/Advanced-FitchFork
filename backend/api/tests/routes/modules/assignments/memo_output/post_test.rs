...
mod tests {
    ...
    fn create_main_java_zip_stub() -> Vec<u8> {
        let mut buf = Vec::new();
        let mut zip = zip::ZipWriter::new(std::io::Cursor::new(&mut buf));
        zip.start_file("Main.java", SimpleFileOptions::default())
            .unwrap();
    ...
    fn create_memo_zip_stub() -> Vec<u8> {
        let mut buf = Vec::new();
        let mut zip = zip::ZipWriter::new(std::io::Cursor::new(&mut buf));
        zip.start_file("memo_content.txt", SimpleFileOptions::default())
            .unwrap();
    ...
    fn setup_input_dirs(module_id: i64, assignment_id: i64) {
        ...
        fs::write(
            memo_dir(module_id, assignment_id).join("memo.zip"),
            create_memo_zip_stub(),
        )
        .unwrap();
        create_execution_config_stub(module_id, assignment_id);
        fs::write(
            makefile_dir(module_id, assignment_id).join("makefile.zip"),
            create_makefile_zip_stub(),
        )
        .unwrap();
        fs::write(
            main_dir(module_id, assignment_id).join("main.zip"),
    ...
    async fn test_post_memo_output_success_as_lecturer() {
        ...
        let memo_path = memo_output_dir(data.module.id, data.assignment.id).join("1.txt");
