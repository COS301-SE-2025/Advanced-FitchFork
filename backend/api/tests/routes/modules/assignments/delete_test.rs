...
mod tests {
    ...
    async fn setup_test_data() -> TestData {
        ...
        let dummy_module = ModuleModel::create(
            "DUMMY9999",
            2024,
            Some("Dummy module for not found test"),
            16
        ).await.unwrap();
        ...
        let file = AssignmentFileModel::save_file(a1.id, module.id, FileType::Spec, "spec.txt", b"spec").await.unwrap();
        let task = AssignmentTaskModel::create(a1.id, 1, "Task 1", "echo Hello").await.unwrap();
        let memo_output = AssignmentMemoOutputModel::save_file(a1.id, task.id, "memo.txt", b"memo").await.unwrap();
        let submission = AssignmentSubmissionModel::save_file(a1.id, student_user.id, 1, "sub.txt", b"sub").await.unwrap();
    ...
    async fn test_delete_assignment_success_as_lecturer() {
        dotenvy::dotenv().expect("Failed to load .env");        let data = setup_test_data().await;
    ...
    async fn test_delete_assignment_success_as_admin() {
        ...
        let db = get_connection().await;
        let found = db::models::assignment::Entity::find_by_id(data.assignments[1].id).one(db).await.unwrap();
        assert!(found.is_none());
    }
