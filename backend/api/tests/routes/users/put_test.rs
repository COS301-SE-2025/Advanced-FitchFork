...
mod tests {
    ...
    async fn setup_test_data() -> TestData {
        ...
        let admin_user = UserModel::create("put_admin", "put_admin@test.com", "adminpass", true).await.expect("Failed to create admin user for PUT tests");
        let non_admin_user = UserModel::create("put_regular", "put_regular@test.com", "userpass", false).await.expect("Failed to create regular user for PUT tests");
        let user_to_update = UserModel::create("target_for_update", "target_original@test.com", "originalpass", false).await.expect("Failed to create target user for update");
