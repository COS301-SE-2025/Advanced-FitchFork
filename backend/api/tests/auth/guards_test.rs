...
mod tests {
    ...
    async fn setup() -> TestContext {
        ...
        let module = ModuleModel::create("TST101", 2025, Some("Guard Test Module"), 1).await.unwrap();
        let admin = UserModel::create("guard_admin", "ga@test.com", "pw", true).await.unwrap();
        let lecturer = UserModel::create("guard_lecturer", "gl@test.com", "pw", false).await.unwrap();
        let tutor = UserModel::create("guard_tutor", "gt@test.com", "pw", false).await.unwrap();
        let student = UserModel::create("guard_student", "gs@test.com", "pw", false).await.unwrap();
        let unassigned = UserModel::create("guard_unassigned", "gu@test.com", "pw", false).await.unwrap();

        UserModuleRoleModel::assign_user_to_module(lecturer.id, module.id, Role::Lecturer).await.unwrap();
        UserModuleRoleModel::assign_user_to_module(tutor.id, module.id, Role::Tutor).await.unwrap();
        UserModuleRoleModel::assign_user_to_module(student.id, module.id, Role::Student).await.unwrap();
    ...
    where
        G: Fn(Path<HashMap<String, String>>, Request<Body>, Next) -> Fut + Clone + Send + Sync + 'static,
        Fut: Future<Output = Result<Response, (StatusCode, Json<ApiResponse<Empty>>)>> + Send + 'static,
    {
        async fn test_handler() -> &'static str { "OK" }

        Router::new()
            .route("/test/{module_id}", get(test_handler).route_layer(middleware::from_fn(guard)))
    ...
    mod test_require_admin {
        ...
        async fn admin_guard_adapter(
            _p: Path<HashMap<String, String>>, req: Request<Body>, next: Next
        ...
        async fn succeeds_for_admin() {
            let ctx = setup().await;
            let app = create_test_router(admin_guard_adapter);
    ...
    mod test_multi_param {
            ...
            Fut: Future<Output = Result<Response, (StatusCode, Json<ApiResponse<Empty>>)>> + Send + 'static,
        {
            async fn test_handler() -> &'static str { "OK" }
            Router::new()
                .route("/test/{module_id}/other/{other_id}", get(test_handler).route_layer(middleware::from_fn(guard)))
                .layer(middleware::from_fn(require_authenticated))
        }
    ...
    mod test_validate_known_ids {
        ...
        async fn fails_when_assignment_not_in_module() {
            let ctx = setup().await;
            let other = ModuleModel::create("OTHER", 2025, None, 1).await.unwrap();
            let assignment = AssignmentModel::create(other.id, "A1", None, AssignmentType::Assignment, Utc::now(), Utc::now()).await.unwrap();
        ...
        async fn succeeds_for_assignment_in_module() {
            let ctx = setup().await;
            let assignment = AssignmentModel::create(ctx.module_id, "A2", None, AssignmentType::Assignment, Utc::now(), Utc::now()).await.unwrap();
            let path = "/modules/{module_id}/assignments/{assignment_id}";
            let app = create_router(path);
            let uri = format!("/modules/{}/assignments/{}", ctx.module_id, assignment.id);
            let res = app.oneshot(build_request(&uri, Some(&ctx.admin_token))).await.unwrap();
        ...
        async fn succeeds_for_task_hierarchy() {
            let ctx = setup().await;
            let assignment = AssignmentModel::create(ctx.module_id, "A3", None, AssignmentType::Assignment, Utc::now(), Utc::now()).await.unwrap();
            let task = TaskModel::create(assignment.id, 1, "T1", "echo").await.unwrap();
        ...
        async fn fails_for_task_not_in_assignment() {
            let ctx = setup().await;
            let assignment1 = AssignmentModel::create(ctx.module_id, "A4", None, AssignmentType::Assignment, Utc::now(), Utc::now()).await.unwrap();
            let assignment2 = AssignmentModel::create(ctx.module_id, "A5", None, AssignmentType::Assignment, Utc::now(), Utc::now()).await.unwrap();
            let task = TaskModel::create(assignment2.id, 1, "T2", "echo").await.unwrap();
            let path = "/modules/{module_id}/assignments/{assignment_id}/tasks/{task_id}";
            let app = create_router(path);
        ...
        async fn succeeds_for_submission_and_file_hierarchy() {
            let ctx = setup().await;
            let assignment = AssignmentModel::create(ctx.module_id, "A6", None, AssignmentType::Assignment, Utc::now(), Utc::now()).await.unwrap();
            let submission = SubmissionModel::save_file(assignment.id, 1, 1, "f.txt", b"test").await.unwrap();
            let file = FileModel::save_file(assignment.id, ctx.module_id, FileType::Spec, "f.txt", b"test").await.unwrap();
        ...
        async fn fails_for_submission_not_in_assignment() {
            let ctx = setup().await;
            let other_assignment = AssignmentModel::create(ctx.module_id, "A7", None, AssignmentType::Assignment, Utc::now(), Utc::now()).await.unwrap();
            let submission = SubmissionModel::save_file(other_assignment.id, 1, 1, "f.txt", b"test").await.unwrap();
