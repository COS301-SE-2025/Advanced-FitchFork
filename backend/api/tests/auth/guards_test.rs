...
mod tests {
        ...
        G: Fn(State<AppState>, Path<HashMap<String, String>>, Request<Body>, Next) -> Fut,
    {
        ...
        Router::new()
            .route(
                "/test/{module_id}",
                get(test_handler).route_layer(middleware::from_fn_with_state(app_state.clone(), guard)),
            )
    ...
    mod test_validate_known_ids {
        ...
        fn create_router(app_state: AppState, path: &'static str) -> Router {
            ...
            Router::new()
                ...
                .route_layer(middleware::from_fn_with_state(app_state, validate_known_ids))
        }
        ...
        async fn fails_when_assignment_not_in_module() {
            let ctx = setup().await;
            let other = ModuleModel::create(ctx.app_state.db(), "OTHER", 2025, None, 1).await.unwrap();
            let assignment = AssignmentModel::create(ctx.app_state.db(), other.id, "A1", None, AssignmentType::Assignment, Utc::now(), Utc::now()).await.unwrap();
        ...
        async fn succeeds_for_assignment_in_module() {
            let ctx = setup().await;
            let assignment = AssignmentModel::create(ctx.app_state.db(), ctx.module_id, "A2", None, AssignmentType::Assignment, Utc::now(), Utc::now()).await.unwrap();
            let path = "/modules/{module_id}/assignments/{assignment_id}";
            let app = create_router(ctx.app_state.clone(), path);
            let uri = format!("/modules/{}/assignments/{}", ctx.module_id, assignment.id);
            let res = app.oneshot(build_request(&uri, Some(&ctx.admin_token))).await.unwrap();
        ...
        async fn succeeds_for_task_hierarchy() {
            let ctx = setup().await;
            let assignment = AssignmentModel::create(ctx.app_state.db(), ctx.module_id, "A3", None, AssignmentType::Assignment, Utc::now(), Utc::now()).await.unwrap();
            let task = TaskModel::create(ctx.app_state.db(), assignment.id, 1, "T1", "echo").await.unwrap();
        ...
        async fn fails_for_task_not_in_assignment() {
            let ctx = setup().await;
            let assignment1 = AssignmentModel::create(ctx.app_state.db(), ctx.module_id, "A4", None, AssignmentType::Assignment, Utc::now(), Utc::now()).await.unwrap();
            let assignment2 = AssignmentModel::create(ctx.app_state.db(), ctx.module_id, "A5", None, AssignmentType::Assignment, Utc::now(), Utc::now()).await.unwrap();
            let task = TaskModel::create(ctx.app_state.db(), assignment2.id, 1, "T2", "echo").await.unwrap();
            let path = "/modules/{module_id}/assignments/{assignment_id}/tasks/{task_id}";
            let app = create_router(ctx.app_state.clone(), path);
